...
class SymbolicStateDfa { 
 ...
 public:
  ...
  /**
   * @brief Returns a symbolic DFA with transitions out of invalid states pruned.
   * 
   * The output symbolic DFA has all transitions out of invalid states redirected to
   * the sink state 0, and the invalid states are removed from the set of final states
   * 
   * \param invalid_states a BDD representing the set of invalid states
   * 
   */
  SymbolicStateDfa get_restriction(const CUDD::BDD& invalid_states) const;
  ...
  /**
   * @brief Construct SymbolicStateDfa from explicit data structures
   * 
   * @param var_mgr Variables at DFA
   * @param automaton_id ID of DFA
   * @param initial_state Initial state as a vector of integers
   * @param final_states Final states function as a BDD
   * @param transition_function Transition function as a vector of BDDs
   * @return SymbolicStateDfa 
   */
  static SymbolicStateDfa from_bdds(std::shared_ptr<Syft::VarMgr> var_mgr,
                                    std::size_t automaton_id,
                                    std::vector<int> initial_state,
                                    CUDD::BDD final_states,
